...
def process_images(
    ...
    resample: Image.Resampling = Image.Resampling.BICUBIC,
    rescale_factor: Optional[float] = 1 / 255.0,
    ...
    if rescale_factor is not None:
        image_arrays = [rescale(image, scale=rescale_factor) for image in image_arrays]
    # Normalize the images to have mean 0 and standard deviation 1
    if image_mean is not None and image_std is not None:
        image_arrays = [
            normalize(image, mean=image_mean, std=image_std) for image in image_arrays
        ]
    # Move the channel dimension to the first dimension. The model expects images in the format [Channel, Height, Width]
    image_arrays = [image.transpose(2, 0, 1) for image in image_arrays]
    return image_arrays
...
class PaliGemmaProcessor:
    ...
    def __init__(self, tokenizer, num_image_tokens: int = 256, image_size: int = 224):
        ...
        self.image_seq_length = num_image_tokens  # 256 = (224 // 14)**2
        ...
        self.image_token_id = tokenizer.convert_tokens_to_ids(self.IMAGE_TOKEN)  # ID
